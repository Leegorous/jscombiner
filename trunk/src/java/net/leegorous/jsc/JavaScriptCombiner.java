...
public class JavaScriptCombiner extends JavaScriptDocument {
	...
	public JavaScriptCombiner(String filePath) throws IllegalPathException, JavaScriptNotFoundException {
		this.setDoc(filePath);
	}

	public JavaScriptCombiner(JavaScriptCombiner linker, File file) throws JavaScriptNotFoundException {
	...
	public void assemble() throws Exception {
		...
		if (this.getLinker()==null) this.findMode();
	...
	protected void findBase() throws Exception {
		Pattern pattern = Pattern.compile("@base ((\\.|([\\w\\.]+/)+)*(\\w+\\.)+cfg\\.js);");
	...
	protected void findClassPaths() throws IOException {
		Pattern pattern = Pattern.compile("@classpath (\\.|([\\w\\.]+/)+);");
		Matcher m = pattern.matcher(this.getContent());
	...
	protected ArrayList readTestConfig(String content) throws IOException {
		Pattern pattern = Pattern.compile("@test ((\\.|([\\w\\.]+/)+)*((\\w+\\.)+js)?);");
		Matcher m = pattern.matcher(content);
	...
	protected void generateAgent() throws Exception {
		String template = StreamReader.read(
				JavaScriptCombiner.class.getResourceAsStream("/AgentTemplate.xml"));
		...
		StringBuffer sb = new StringBuffer();
		if (this.isAgentMode()) imports = this.getImportedFiles();
		else {
		...
		for (int i=0, j=imports.size(); i<j; i++) {
			...
			sb.append(translate2RelatePath(this.getDoc(), (File) imports.get(i)));
			sb.append("\"></script>\r\n");
		}
	...
	public static String getCharset(File file) throws Exception {
		String charset = null;
		FileCharsetDetector detector = new FileCharsetDetector(new FileInputStream(file));
		if ((charset=detector.getResult())==null) 
			if (detector.getProbableResult()!=null && detector.getProbableResult().length>0) 
	...
	protected void setDoc(String filePath) throws IllegalPathException, JavaScriptNotFoundException {
		if (Pattern.matches("^(/[\\w.]+)+|[\\w.]+$", filePath)) {
			//this.filePath = filePath;
		} else if (Pattern.matches("^[a-zA-Z]:(\\\\[\\w.]+)+$", filePath)) {
			//this.filePath = filePath;
		} else {
				...
				if (!item.equals(".")) {
					if (item.equals(".."))
						...
						if (subs.length>0) {
							...
							result = subs;
						} else throw new IOException("Cannot find a file named \""
								+item+"\" in \""+target.getAbsolutePath()+"\"");
					}
				}
			}
		} else {
	...
	public static String translate2RelatePath(File standard, File file) throws IOException {
		if (standard.isFile()) standard = standard.getParentFile();
		...
		if (stack.size()>0) result.append((String) stack.pop());
