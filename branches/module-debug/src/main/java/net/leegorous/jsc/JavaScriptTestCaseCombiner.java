...
public class JavaScriptTestCaseCombiner extends JavaScriptDocument {
	...
	public JavaScriptTestCaseCombiner(File file, Set classpath, File jsUnit)
			throws JavaScriptNotFoundException {
		super(null, file, classpath);
		setJsUnit(jsUnit);
	}
	...
	public void assemble() throws Exception {
		...
		// File testCaseFile = new
		// File(this.getDoc().getAbsolutePath().replaceAll("\\.js", ".html"));
		...
		for (int i = 0, j = imports.size(); i < j; i++) {
			...
			sb.append(JavaScriptCombiner.translate2RelatePath(this.getDoc(),
					(File) imports.get(i)));
			sb.append("\"></script>\r\n");
		}
		template = template.replaceAll("\\{!\\[testCaseScripts]}", sb
				.toString());
	...
	protected static String getTemplate() throws Exception {
		return StreamReader.read(JavaScriptTestCaseCombiner.class
				.getResourceAsStream(TESTCASE_TEMPLATE_PATH));
	}

	protected static void saveTestCase(File file, String content)
			throws Exception {
